<template>
  <div id="releases">
    ...
    <h2>BSD</h2>
    <div class="chart-container">
      <canvas id="bsd-chart" width="400" height="400"></canvas>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    bsdReleases() {
      return this.getReleases(this.isBSD);
    },
    linuxReleases() {
  ...
  methods: {
    ...
    isBSD(asset) {
      return asset.name.includes("bsd");
    },
    isLinux(asset) {
        ...
        datasets: [
          {
            data: [
              ...
              this.bsdReleases[this.bsdReleases.length - 1].downloads
            ],
            backgroundColor: ["yellow", "#dddddd", "#2d89ef", "red"]
          }
        ],
        labels: ["Linux", "MacOS", "Windows", "BSD"]
      };
      let options = {};
  ...
  async mounted() {
    ...
    let url = "https://api.github.com/repos/tendermint/tendermint/releases";
    ...
    this.drawChart("#bsd-chart", this.bsdReleases);
